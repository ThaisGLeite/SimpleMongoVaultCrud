...
while ! docker logs docker_vault_1 2>&1 | grep -q 'Vault server started! Log data will stream in below:'
do
  sleep 1
done

# Initialize the Vault server and store the keys in keys.txt
docker exec docker_vault_1 vault operator init > keys.txt
...
# Unseal the Vault
docker exec docker_vault_1 vault operator unseal $KEY1
docker exec docker_vault_1 vault operator unseal $KEY2
docker exec docker_vault_1 vault operator unseal $KEY3
...
docker exec -e VAULT_TOKEN=$ROOT_TOKEN docker_vault_1 vault secrets enable -path=secret kv
docker exec -e VAULT_TOKEN=$ROOT_TOKEN docker_vault_1 vault kv put secret/data/mongodb data='{"username":"thaisdev","password":"DevEnv123"}'
